...
// Polynom
public class A1_POLIB_0 extends Block_Prototype_Fxy {

    static final long serialVersionUID = 1L;

    public A1_POLIB_0() {
        super("A1_POLIB_0", 0, 30);
    }
    double val = -0.5;
    double min = 0;
    double max = 2.0;
    ...
    public double f(double x, double y) {
        ...
        double c1 = Math.sin(ax1);
        double c2 = Math.sin(ay1);
        double c3 = Math.sin(axy);
        double r = ax*ay*axy*(params[25] * c1 +  params[26] *c2 + params[27] * c3) ;
        if (r > min && r < max) {
            return r;
        }
        return val;
