...
//////////////////////////////////////////////////IMAGES
    ...
    public static void saveImageJPEG(FW_ImageFrame f) {
        saveImage(f, "jpg");
    }

    static void saveImagePNG(FW_ImageFrame f) {
        saveImage(f, "png");
    }
...
// }
    ...
    public static void restoreSet(FW_Desktop ds) {
        initSetFileChooser();
...
///////////SAVE IMAGES OF MODELS //////////////////////////////////
    static JFileChooser setx = null;

    static void initImageFolderChooser() {
        if (setx != null) {
            return;
        }
        setx = new JFileChooser();
        setx.setCurrentDirectory(new File(FW_Parm.rootDir));
        setx.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
        setx.setDialogTitle("Select Folder For Images");
    }

    static void SaveImagesToFolder(FW_Desktop ds) {
        initImageFolderChooser();
        int rs = setx.showOpenDialog(ds);
        if (rs == JFileChooser.APPROVE_OPTION) {
            try {
                File folder = setx.getSelectedFile();
                ...
                FW_Desktop dsctop =(FW_Desktop)FW_Parm.getBlockDesktop();
                ...
                for (Iterator iter = ls.iterator(); iter.hasNext();) {
                    ...
                    // f.paintButton_actionPerformed(null);
                    f.paint_synchronized();
                    try {
                        String type = "jpg";
                        File file = new File(folder,b.getName());
                        file = FW_Utils.setExtension(file, type);
                        System.out.println(">>>>" + file.getPath()+file.getPath());
                        //dsctop.removeAllFrames(); May be added for memory save
                        ImageIO.write(f.ip.img, type, file);
                    } catch (Exception e) {
                        System.out.println(e);
                        e.printStackTrace();
